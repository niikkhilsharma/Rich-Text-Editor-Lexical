...
import ColorPlugin from "../ColorPlugin/ColorPlugin.jsx";
import ListPlugin from "../ListPlugin/ListPlugin.jsx";
import ToolbarHeading from "./../../components/ToolbarHeading";
import { $isListNode, ListNode } from "@lexical/list";
import { $isCodeNode, getDefaultCodeLanguage } from "@lexical/code";
import { InsertTableDialog } from "../TablePlugin/TablePlugin.jsx";
import PageBreakPlugin from "../PageBreakPlugin/PageBreakPlugin.jsx";
import FieldCodePlugin from "../FieldCodePlugin/FieldCodePlugin.jsx";
import BannerPlugin from "../BannerPlugin.jsx";
import useModal from "./../../hooks/useModal";
import { cn } from "../../../utils/utils.js";
import { LayoutPlugin } from "../LayoutPlugin/LayoutPlugin.jsx";
import InsertLayoutDialog from "../LayoutPlugin/InsertLayoutDialog.jsx";
import { InsertImageDialog } from "../ImagePlugin/index.jsx";
import BannerFormattingPlugin from "./../CustomBannerPlugin";
import { Dash, Image, LayoutSplit, PlusLg, Table } from "react-bootstrap-icons";
import { getHeadingSizeByTagName } from "../utils.js";
...
export default function ToolbarPlugin() {
  ...
  const updateToolbar = useCallback(() => {
    ...
    if ($isRangeSelection(selection)) {
      ...
      const textNodeHasSize =
        ...
        parseInt($getSelectionStyleValueForProperty(selection, "font-size"));
      const isParagraphNode = $isParagraphNode(
        $getNearestNodeOfType(selection.anchor.getNode(), ParagraphNode)
      );
      const isHeadingNode = $isHeadingNode(
        $getNearestNodeOfType(selection.anchor.getNode(), HeadingNode)
      );
  ...
  return (
    <div
      className={"w-1/5 " + "min-w-60 p-2 pb-4"}
      style={{ backgroundColor: "#DEEFFB" }}
    >
      ...
      {activeTab === "STYLES" && (
        <>
          ...
          <div className="bg-[#f1f8fd] mt-2 rounded-md">
            <ToolbarHeading
              variant="secondary"
              className={"px-3 py-2 border-b-2 border-gray-200"}
            >
              Text
            </ToolbarHeading>
            <div className="p-2">
              <div className="border flex justify-between items-center rounded-md bg-white">
                ...
                <p className="w-full text-center">
                  {selectionMap[RichTextAction.FontSize]}
                </p>
                <button
                  className={cn(
                    "rounded-md p-2 border hover:opacity-85 hover:bg-slate-200 h-8 w-8 aspect-square"
              ...
              <div className="flex gap-2 flex-wrap">
                {RICH_TEXT_OPTIONS.map(({ id, icon, label, fontSize }) => (
                  <button
                    ...
                    className={`rounded-md p-2 border flex justify-center items-center hover:opacity-85 hover:bg-slate-200 h-8 w-8 aspect-square ${
                      selectionMap[id]
                        ? "border-none bg-blue-800 text-white"
                        : ""
                    }`}
                    key={id}
                ...
                <button
                  className={cn(
                    "rounded-md p-2 border hover:opacity-85 hover:bg-slate-200 h-8 w-8 text-xs"
                  )}
                  title="Insert Layout"
                  onClick={() => {
                    showModal("Insert Layout", (onClose) => (
                      <InsertLayoutDialog
                        activeEditor={activeEditor}
                        onClose={onClose}
                      />
