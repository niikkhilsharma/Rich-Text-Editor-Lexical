...
import { getThemeSelector } from '@/lib/utils'
...
function TableHoverActionsContainer({ anchorElem }) {
			...
			if (tableDOMElement) {
				...
				if (hoveredRowNode) {
					...
					setPosition({
						...
						top: tableElemBottom - editorElemY + (tableHasScroll ? (isMac ? 5 : 16) : 5),
	...
	return (
		<>
			...
			{isShownColumn && (
				<button className={`${getTheme()?.tableAddColumns}`} style={{ ...position }} onClick={() => insertAction(false)} />
			)}
		</>
	)
}
